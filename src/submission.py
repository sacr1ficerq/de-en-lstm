...
def get_bleu_beam_search(model, dataloader, vocab_trg, filenames, device='cuda', raw_dataset=None):
    ...
    with open(pred_filename, 'w', encoding='utf-8') as f_pred:
        with torch.no_grad():
            for batch_idx, (src, _) in enumerate(tqdm(dataloader)):
                # predictions = model.inference(src, max_len=None, device=device) # batch
                predictions = model.inference_beam(src, device=device) # batch
                for i in range(len(src)):
                    t = None
                    if raw_dataset != None:
                        t = raw_dataset[batch_idx * dataloader.batch_size + i]
...
def make_submission_beam_search(model, submission_loader, vocab_trg, filenames, device='cuda', raw_dataset=None):
    with open(filenames['submission_trg'], 'w', encoding='utf-8') as f_pred:
        with torch.no_grad():
            for batch_idx, a in enumerate(tqdm(submission_loader)):
                predictions = model.inference_beam(a, max_len=None, device=device) # batch
                for i in range(len(a)):
                    t = None
                    if raw_dataset:
                        t = raw_dataset[batch_idx * submission_loader.batch_size + i]
